...
class MAFAST(Strategy):
    ...
    def __init__(
        ...
        self.duo_could_flag = False
        self.kong_could_flag = False

        self.shaking_detecting = False
        self.shaking_flag = False
        ...
        self.shaking_count = self.shaking_count_num
    ...
    def generate_signal(self, dt: datetime):
        ...
        new_orders = []
        min_data = self.broker.get_candles(instrument=self.instrument, granularity="1min", count=15, cut_yesterday=True)
        ...
        new_ma_short = ma_short.iloc[-5:]
        new_ma_long = ma_long.iloc[-5:]
        ma_delta = new_ma_short - new_ma_long
        ma_delta_speed = []
        for i in range(len(ma_delta)):
            if i == 0:
                continue
            ma_delta_speed.append(ma_delta.iloc[i] - ma_delta.iloc[i-1])

        if ma_delta.iloc[-1] * ma_delta.iloc[-2] < 0:
            self.shaking_detecting = True
            self.shaking_count = self.shaking_count_num

        if self.shaking_detecting:
            self.shaking_count = self.shaking_count - 1
            if self.shaking_count == 0:
                self.shaking_detecting = False
            if ma_delta.iloc[-1] * ma_delta.iloc[-2] < 0:
                self.shaking_flag = True
        ...
        if self.duo_stopping:
            if last_short > last_long:
                self.duo_stopping = False
                self.duo_could_flag = False
                if ma_delta_speed[-1] < 0:
                    print(f"{self.instrument} CLOSING LONG! enter: {self.duo_enter_point} exit: {current_point} speed: {ma_delta_speed[-1]} short: {last_short} long: {last_long}")
        ...
        if self.kong_stopping:
            if last_short < last_long:
                ...
                self.kong_could_flag = False
                if ma_delta_speed[-1] > 0:
                    print(f"{self.instrument} CLOSING SHORT! enter: {self.kong_enter_point} exit: {current_point} speed: {ma_delta_speed[-1]} short: {last_short} long: {last_long}")
        ...
        if not self.shaking_flag:
            if not self.duo_stopping:
                if not self.duo_flag:
                    if last_short < last_long - self.diff :
                        self.duo_could_flag = True
                    if 0 > ma_delta_speed[-1] > ma_delta_speed[-2] and self.duo_could_flag:
                        print(f"{self.instrument} OPENING LONG! enter: {current_point} short: {last_short} long: {last_long} speed-1: {ma_delta_speed[-1]} speed-2: {ma_delta_speed[-2]}")
            ...
            if not self.kong_stopping:
                if not self.kong_flag:
                    if last_short > last_long + self.diff:
                        self.kong_could_flag = True
                    if 0 < ma_delta_speed[-1] < ma_delta_speed[-2] and self.kong_could_flag:
                        print(f"{self.instrument} OPENING SHORT! enter: {current_point} short: {last_short} long: {last_long} speed-1: {ma_delta_speed[-1]} speed-2: {ma_delta_speed[-2]}")
        ...
        '''if self.duo_flag:
            if self.current_point > self.duo_enter_point + self.take_profit:
                print(f"{self.instrument} TAKEN PROFIT LONG! enter: {self.duo_enter_point} exit: {current_point}")
                duo_exit_point = current_point - self.trade_offset
                self.broker.relog()
                new_order = Order(
                    instrument=self.instrument,
        ...
        if self.kong_flag:
            if self.current_point < self.kong_enter_point - self.take_profit:
                print(f"{self.instrument} TAKEN PROFIT SHORT! enter: {self.kong_enter_point} exit: {current_point}")
                kong_exit_point = current_point + self.trade_offset
                self.broker.relog()
                new_order = Order(
                    instrument=self.instrument,
